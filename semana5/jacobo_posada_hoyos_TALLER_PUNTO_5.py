...
# Devuelve un array de tamanio 4. La primera posicion indica si es valido, con True y si es invalido con False.
# La segunda posicion contiene el nombre completo en caso de ser valida la linea, de lo contrario vacio.
# La tercera posicion contiene el salario en caso de ser valida la linea, de lo contrario vacio.
# La cuarta posicion contiene el cargo del empleado.
def validar_linea(linea_por_validar, numero_linea):	
	...
	# Validar que el cargo sea de tipo alfabetico
	# Validacion 7 (Va7)
	cargo_empleado = arreglo_campos[2]
	try: 
		cargo_empleado = int(arreglo_campos[2])
		guardar_error("El cargo del empleado " + arreglo_campos[2] + " debe ser alfabetico! Revisar linea numero " + str(numero_linea) + " de archivo de nomina.")
		array_respuesta[0] = False
		return array_respuesta
	except ValueError:
		array_respuesta[3] = cargo_empleado
...
# Se crea array nomina con dimensiones 3 columnas y tantas filas como empleados o numero de lineas en el archivo de nombres.txt, es decir, numero_lineas_nomina
# nomina[indice][0] -> Nombre completo empleado
# nomina[indice][1] -> Salario empleado
# nomina[indice][2] -> Cargo empleado
...
for x in range(0, numero_lineas_nomina):
	...
	# Se almacena el cargo
	nomina[x][2] = linea_validada[3]
	if nomina[x][2][len(nomina[x][2])-1] == "\n":
		nomina[x][2] = nomina[x][2][0:len(nomina[x][2])-1]
...
contador_ingenieros = 0
contador_mensajeros = 0
contador_tecnicos = 0
contador_secretarias = 0
contador_operarios = 0
acumulador_ingenieros = 0
acumulador_mensajeros = 0
acumulador_tecnicos = 0
acumulador_secretarias = 0
acumulador_operarios = 0
total_nomina = 0
for z in range(0, numero_lineas_nomina):
	# Se almacena salario base para realizar calculos
	salario_base = int(nomina[z][1])
	cargo_empleado = (nomina[z][2])
	# Se suman y se cuentan las nominas por tipo de empleado y total de nomina
	# Se cuentan los empleados por tipo.
	
	total_nomina = total_nomina + salario_base
	
	guardar_log("Acumulando salario para tipo de empleado : "+cargo_empleado+"")
	
	guardar_log("Acumulando valor para nomina total")
	...
	if cargo_empleado == "Ingeniero" or cargo_empleado == "Ingeniera":
		acumulador_ingenieros = acumulador_ingenieros + costo_total_empresa 
		contador_ingenieros = contador_ingenieros + 1
	
	if cargo_empleado == "Mensajero":
		acumulador_mensajeros = acumulador_mensajeros + costo_total_empresa
		contador_mensajeros = contador_mensajeros + 1
		
	if cargo_empleado == "Tecnico" :
		acumulador_tecnicos = acumulador_tecnicos + costo_total_empresa 
		contador_tecnicos = contador_tecnicos + 1
		
	if cargo_empleado == "Secretaria":
		acumulador_secretarias = acumulador_secretarias + costo_total_empresa
		contador_secretarias = contador_secretarias + 1 
		
	if cargo_empleado == "Operario" or cargo_empleado == "Operaria":
		acumulador_operarios = acumulador_operarios + costo_total_empresa
		contador_operarios = contador_operarios + 1

# Calculo de los promedios por tipo y total

if contador_ingenieros != 0 and contador_mensajeros != 0 and contador_operarios != 0 and contador_secretarias != 0 and contador_tecnicos != 0 :
	promedio_ingenieros = acumulador_ingenieros / contador_ingenieros
	promedio_mensajeros = acumulador_mensajeros / contador_mensajeros
	promedio_secretarias = acumulador_secretarias / contador_secretarias
	promedio_operarios = acumulador_operarios / contador_operarios
	promedio_tecnicos = acumulador_tecnicos / contador_tecnicos
	promedio_total_nomina = total_nomina / numero_lineas_nomina

guardar_log("Calculados promedios totales y por cargos (mensual y anual)")
...
escribir_linea_archivo(nombre_archivo_liquidacion, "Total Nomina Mensual = "+str(total_nomina)+" * Total Ingenieros = "+str(acumulador_ingenieros)+" * Total Operarios = "+str(acumulador_operarios)+" * Total Secretarias = "+str(acumulador_secretarias)+" * Total Tecnicos = "+str(acumulador_tecnicos)+" * Total Mensajeros = "+str(acumulador_mensajeros)+"\n")
escribir_linea_archivo(nombre_archivo_liquidacion, "Promedio Nomina Mensual ="+str(promedio_total_nomina)+" * Promedio Ingenieros Mensual= "+str(promedio_ingenieros)+" * Promedio Operarios Mensual= "+str(promedio_operarios)+" * Promedio Secretarias Mensual= "+str(promedio_secretarias)+" * Promedio Tecnicos Mensual= "+str(promedio_tecnicos)+" * Promedio Mensajeros Mensual= "+str(promedio_mensajeros)+"\n")
escribir_linea_archivo(nombre_archivo_liquidacion, "Total Nomina Anual = "+str(total_nomina)+" * Total Ingenieros Anual = "+str(acumulador_ingenieros*12)+" * Total Operarios Anual = "+str(acumulador_operarios*12)+" * Total Secretarias Anual = "+str(acumulador_secretarias*12)+" * Total Tecnicos Anual = "+str(acumulador_tecnicos*12)+" * Total Mensajeros Anual = "+str(acumulador_mensajeros*12)+"\n")
