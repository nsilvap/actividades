...
for z in range(0, numero_lineas_nomina):
	...
	if nomina[z][2].startswith("ingeniero"):
		t_ingenieros = t_ingenieros + costo_total_empresa
		contador_ing = contador_ing + 1
	elif nomina[z][2] == "tecnico":
		t_tecnicos = t_tecnicos + costo_total_empresa
		contador_tec = contador_tec +1
	elif nomina[z][2] == "operario":
		t_operarios = t_operarios + costo_total_empresa
...
contenido_linea = ("total nomina",(t_ingenieros+t_tecnicos+t_operarios),"Ingenieros", t_ingenieros, "Tecnicos", t_tecnicos, "Operarios", t_operarios)
formato_linea = "%s: %.2f    |%s: %.2f|     %s: %.2f|     %s: %.2f|     %s: %.2f|\n\n"
escribir_linea_archivo(nombre_archivo_liquidacion, formato_linea % (contenido_linea))
contenido_linea = ("Salario promedio para todos los empleados", (float(suma_nomina)/float(numero_lineas_nomina)), "Salario promedio ingenieros", (float(t_ingenieros)/float(contador_ing)), "Salario promedio para tecnicos", (float(t_tecnicos)/float(contador_tec)), "Salario promedio operarios", (float(t_operarios)/float(contador_ope)))
formato_linea = "%s: %.2f | %s: %.2f | %s: %.2f | %s: %.2f | %s: %.2f|\n\n"
escribir_linea_archivo(nombre_archivo_liquidacion, formato_linea % (contenido_linea))
contenido_linea = ("Total nomina todo el anio", (suma_nomina*12), "Nomina anual para ingenieros", (t_ingenieros*12), "Nomina anual para tecnicos", (t_tecnicos*12), "Nomina anual para operarios", (t_operarios*12))
formato_linea = "%s: %.2f | %s: %.2f | %s: %.2f | %s: %.2f | %s: %.2f|\n\n"
escribir_linea_archivo(nombre_archivo_liquidacion, formato_linea % (contenido_linea))
