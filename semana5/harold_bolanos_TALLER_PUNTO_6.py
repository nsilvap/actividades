...
a_deducciones = tuple(leer_lineas_archivo("deducciones.txt"))
a_apropiaciones = tuple(leer_lineas_archivo("apropiaciones.txt"))

# Se validan los archivos apropiaciones y deducciones

for u in range(0,2):
	dedvalidada = validar_linea_dedapr(a_deducciones[u], u+1)
	aprvalidada = validar_linea_dedapr(a_apropiaciones[u], u+1)
...
for z in range(0, numero_lineas_nomina):
	...
	aporte_salud_empleado = float(a_deducciones[0])*salario_base

	if(debitar_fondo_solidaridad):
		guardar_log("Empleado paga fondo solidaridad pensional ...");
		aporte_fondo_solidaridad = float(a_deducciones[1])*salario_base		

	# Porcentaje aporte de salud realizado por el empleado 4%
	guardar_log("Calculando aporte pension empleado ...");
	aporte_pension_empleado = float(a_deducciones[2])*salario_base
