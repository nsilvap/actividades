...
# Desarrollado por Nicolas Silva Palacios
...
def validar_linea_salarios (linea_por_validar, numero_linea):
	...
	try:
		...
		if salario < 0:
			guardar_error("El valor de salario ingresado no es positivo. Revise la linea "  str(numero_linea) "")
			array_respuesta[0] = False
			return array_respuesta
	except ValueError:
			array_respuesta[0] = False
			return array_respuesta
			guardar_error("El valor de salario ingresado no es numerico. Revise la linea "  str(numero_linea) "")
	array_respuesta[0]=True
...
# Validar que el numero de argumentos sea igual a 3, garantizando que se haya el nombre del archivo de nomina.
# Validacion 1 (Va1)
if (cantidad_argumentos != 3):	
	terminar_programa("Numero de argumentos incorrecto. Debe suministrar un argumento con los nombres de archivos de nombres y salario.")
...
# Validar que los archivos de nombres y salarios suministrados como argumentos tenga extension .txt.
# Validacion 2 (Va2)
if (nombre_archivo_nombres.endswith(".txt") == False):
	if (nombre_archivo_salarios.endswith(".txt") == False):
		terminar_programa("Ambos archivos no tienen extension .txt")
	else:
...
if (numero_lineas_nombres != numero_lineas_salarios):
	terminar_programa("Los archivos de nombres y salarios deben de contener el mismo numero de lineas ")
guardar_log("Numero de lineas de archivos OK")
...
# Se crea array liquidacion con dimensiones 13 columnas y tantas filas como empleados o numero de lineas en el archivo de salarios.txt, es decir, numero_lineas_salarios
...
liquidacion = [[columnas for columnas in range(13)] for filas in range(numero_lineas_salarios)]

# Ciclo 1 para almacenar la informacion en el array de nomina
guardar_log("Cargando nomina en memoria...");
for x in range(0, numero_lineas_salarios):
...
for z in range(0, numero_lineas_salarios):
