...
# Validar que el numero de argumentos sea igual a 3, garantizando que se haya el nombre del archivo de nombres y salarios.
# Validacion 1 (Va1)
if (cantidad_argumentos != 3):	
	terminar_programa("Numero de argumentos incorrecto. Debe suministrar un argumento con el nombre de archivo de nombres y salarios.")
...
# Validar que el archivo de nombres y salarios suministrados como argumentos tengan extension .txt.
# Validacion 2 (Va2)
if (nombre_archivo_nombres.endswith(".txt") == False):
	terminar_programa("El archivo de nombres no tiene extension .txt!")
	
guardar_log("Extension de archivo de nombres OK")

if (nombre_archivo_salarios.endswith(".txt") == False):
	terminar_programa("El archivo de salarios no tiene extension .txt!")
	
guardar_log("Extension de archivo de salarios OK")
...
guardar_log("Archivo de salarios leido OK")
...
if (numero_lineas_salarios < numero_minimo_lineas):
	terminar_programa("El archivo de salarios debe contener como minimo " + str(numero_minimo_lineas) + " lineas!")
...
# Se crea array nombres con dimensiones 2 columnas y tantas filas como empleados o numero de lineas en el archivo de nombres.txt, es decir, numero_lineas_nombres
...
salarios = [[columnas for columnas in range(2)] for filas in range(numero_lineas_salarios)]
...
for x in range(0, numero_lineas_nombres):
	linea_validada = validar_linea(lineas_archivo_nombres[0], x+1)
	

	guardar_log("Procesando linea " + str(x+1));
	
	# Se almacena nombre completo
	nombres[0] = linea_validada[1]
	
	# Se almacena salario
	salarios[0] = linea_validada[2]
