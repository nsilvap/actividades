...
lineas_archivo_nombre = tuple(leer_lineas_archivo(nombre_archivo_nombre))
...
numero_lineas_nombre = len(lineas_archivo_nombre)
...
# Validacion 3 (Va3)
if (numero_lineas_nombre < numero_minimo_lineas):
...
nomina = [[columnas for columnas in range(2)] for filas in range(numero_lineas_nombre)]
...
for x in range(0, numero_lineas_nombre):
	linea_validada = validar_linea(lineas_archivo_nombre[x], x+1)
...
for z in range(0, numero_lineas_nombre):
...
for w in range(0, numero_lineas_nombre):
	contenido_linea = (
		nomina[w][0], 
		int(nomina[w][1]), 
		liquidacion[w][0],
