# AyudanteNomina v2.0.1
# Programa que calcula la nomina de una empresa a partir de un archivo de texto con los nombres y un archivo de texto con los salarios,
# Al final guarda la nomina en el archivo liquidacion.txt. Se guarda registro de errores en errores.txt y registro de operacion en archivo log.txt.
...
#Funcion para validar lineas de archivo nombres

def validar_linea_nombres(linea_por_validar, numero_linea):	
	array_respuesta_n = [0 for x in range(2)]
	#Validar que el nombre contenga la longitud requerida
	# Validacion Va4
	try:
		arreglo_nombre = linea_por_validar.split(" ")
		len(arreglo_nombre) < 2 or len(arreglo_nombre) > 5
		array_respuesta_n[1] = arreglo_nombre
	except ValueError:	
		guardar_error("El nombre" + arreglo_nombre + " No cumple la longitud requerida! Revisar linea numero " + str(numero_linea) + " de archivo de nombres.")
		array_respuesta_n[0] = False
		return array_respuesta
		
	array_respuesta_n[0] = True
	return array_respuesta_n

#Funcion para validar lineas de archivo salarios

def validar_linea_salarios(linea_por_validar, numero_linea):	
	array_respuesta_s = [0 for x in range(2)]
	
	# Separar la linea por el simbolo (token) *
	arreglo_salario = linea_por_validar
	
	# Validar que el salario sea de tipo numerico
	# Validacion  (Va5)
	try:
		salario = int(arreglo_salario)								
		array_respuesta_s[1] = salario
	except ValueError:	
		guardar_error("El valor de salario " + arreglo_salario + " no puede convertirse a entero! Revisar linea numero " + str(numero_linea) + " de archivo de salarios.")
		array_respuesta_s[0] = False
		return array_respuesta_s
		
	array_respuesta_s[0] = True
	return array_respuesta_s
...
# Validar que el numero de argumentos sea igual a 3, garantizando que se haya el nombre del archivo de nombres y salario base.
# Validacion 1 (Va1)
if (cantidad_argumentos != 3):	
	terminar_programa("Numero de argumentos incorrecto. Debe suministrar un argumento con el nombre de archivo de nombes y nombre de archivo salarios.")

guardar_log("Numero de argumentos OK")

nombre_archivo_nombres = sys.argv[1]
nombre_archivo_salario_base = sys.argv[2]

# Validar que el archivo de nomina suministrado como argumento tenga extension .txt.
# Validacion Va2 y Va3

if (nombre_archivo_nombres.endswith(".txt") == False):
	terminar_programa("El archivo de nombres no tiene extension .txt!")
	
guardar_log("Extension de archivo nombres OK")
	
if (nombre_archivo_salario_base.endswith(".txt") == False):
	terminar_programa("El archivo de salarios no tiene extension .txt!")
	
guardar_log("Extension de archivo salarios OK")

# Variable que almacena las lineas del archivo nombres. su contenido como tal.
...
guardar_log("Archivo de nombres leido OK")

# Validar que el archivo tenga el minimo numero de lineas.
# Validacion 3 (Va3)
if (numero_lineas_nombres < numero_minimo_lineas):
	terminar_programa("El archivo de nombres debe contener como minimo " + str(numero_minimo_lineas) + " lineas!")
	
# Variable que almacena las lineas del archivo, su contenido como tal.
lineas_archivo_salario_base = tuple(leer_lineas_archivo(nombre_archivo_salario_base))

# Variable que almacena el numero de lineas del archivo
numero_lineas_salario_base = len(lineas_archivo_salario_base)

guardar_log("Archivo de salario base leido OK")

if (numero_lineas_salario_base < numero_minimo_lineas):
	terminar_programa("El archivo de salario base debe contener como minimo " + str(numero_minimo_lineas) + " lineas!")

#Validar que El numero de lineas de archivo salario base y nombres sea igual
 # Va7
if(numero_lineas_nombres != numero_lineas_salario_base):
	terminar_programa("El archivo de nombresdebe contener el mismo numero de lineas que el archivo salario base")
...
array_nomina = [[columnas for columnas in range(3)] for filas in range(numero_lineas_salario_base)]
...
liquidacion = [[columnas for columnas in range(13)] for filas in range(numero_lineas_salario_base)]


# Ciclo 1 para almacenar la informacion en el array de salarios desde archivos nombres y salarios.

guardar_log("Cargando Salarios en memoria...");
for x in range(0, numero_lineas_salario_base):
	linea_validada_sal = validar_linea_salarios(lineas_archivo_salario_base[x], x+1)
	linea_validada_nom = validar_linea_nombres(lineas_archivo_nombres[x], x+1)

	if(linea_validada_sal[0] == False):
		terminar_programa("Error en archivo de salario base!")

	guardar_log("Procesando linea de archivos nombres" + str(x+1));
	guardar_log("Procesando linea de archivos salarios" + str(x+1));
	
	# Se almacena nombre completo
	array_nomina[x][0] = linea_validada_nom[1]
	
	array_nomina[x][1] = linea_validada_sal[1]

guardar_log("Salarios y nombres cargados en memoria!");
# Fin de ciclo 1

# Ciclo 3 para realizar el calculo de la liquidacion
guardar_log("Calculando liquidacion...");
for z in range(0, numero_lineas_salario_base):
	# Se almacena salario base para realizar calculos
	salario_base = int(array_nomina[z][1])
...
for w in range(0, numero_lineas_salario_base):
	contenido_linea = (
		array_nomina[w][0], 
		int(array_nomina[w][1]), 
		liquidacion[w][0],
		liquidacion[w][1],
		liquidacion[w][2],
		liquidacion[w][3],
